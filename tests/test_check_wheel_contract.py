...
def _wheel(
    ...
    *,
    ...
    writer.writerow(["logseq_matryca_parser-1.6.0.dist-info/METADATA", "", ""])
    writer.writerow(["logseq_matryca_parser-1.6.0.dist-info/RECORD", "", ""])

    with zipfile.ZipFile(path, "w") as archive:
        if include_marker:
            archive.writestr(MARKER, "")
        archive.writestr("logseq_matryca_parser-1.6.0.dist-info/METADATA", metadata)
        archive.writestr("logseq_matryca_parser-1.6.0.dist-info/RECORD", record.getvalue())
    return path


def test_source_version_reads_lightweight_version_module() -> None:
    assert source_version() == "1.6.0"


def test_valid_wheel_contract(tmp_path: Path) -> None:
    wheel = _wheel(tmp_path / "package.whl")

    assert check_wheel(wheel, "1.6.0") == []
...
def test_wheel_contract_reports_version_mismatch(tmp_path: Path) -> None:
    ...
    assert check_wheel(wheel, "1.6.0") == [
        "wheel version '1.5.0' does not match source version '1.6.0'"
    ]
