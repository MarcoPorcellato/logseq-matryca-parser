...
ordinary dictionaries for compatibility. Published dictionaries and backlink
lists are immutable by repository convention: a later candidate must copy them
before mutation. No supported library mutation may alter a previously
published dictionary or list.

`invalidate_and_reload_page()` acquires the lock, captures the current
snapshot, and constructs the candidate from private copies. Parsing,
enrichment, stale-node removal, targeted backlink repair, title-map
construction, and validation all finish before publication. An exception at
any point leaves every current field untouched.

Publication assigns `pages`, `_node_registry`, `_backlink_registry`,
`_lower_title_map`, and `_index_diagnostics` while the same lock excludes all
supported read methods. Direct `graph.pages` access receives either the old or
new mapping object because publication replaces the reference and never mutates
the published mapping afterward. Separate raw mapping reads are not a public
transaction; callers needing coherent graph operations must use supported
methods.
...
Candidate backlink lists must be deep-copied. When a title or alias changes,
recompute contributions for every source node whose wikilink resolution changes
between the old and candidate page maps. A full node scan for affected links is
acceptable; rebuilding all backlink entries is not.
...
5. **Cross-process file locking.** Deferred; the accepted scope is one vault in
   one process.
...
- Do not expose the lock, candidate, callback queue, or dispatcher from the
  package root.
