...
| Current milestone | M5 in delivery | Local code head `41486b3` on `agent/parser-privacy-assurance-m5`; it is not a publication, hosted-check, merge, or release claim |
...
- [ ] The privacy-safe local graph tool is delivered or explicitly rejected with
  recorded reasons. A local implementation head exists at `41486b3`, but
